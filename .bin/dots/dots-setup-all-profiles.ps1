...
if($IsWindows) {
  ...
  $windows = "$HOME/.config/powershell/WindowsPowerShell/Microsoft.PowerShell_profile.ps1";
  $vscode  = "$HOME/.config/powershell/Microsoft.VSCode_profile.ps1";
}
...
##
Write-Output                              `
  "if(Test-Path '$pwsh') { . '$pwsh'; }"  `
    | Out-File -FilePath "$windows";

Write-Output                              `
  "if(Test-Path '$pwsh') { . '$pwsh'; }"  `
    | Out-File -FilePath "$vscode";
